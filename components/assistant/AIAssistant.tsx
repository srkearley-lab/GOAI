'use client';
/* ============================================================
   GO AI — guided AI Assistant (rule-based) + WhatsApp toggle.
   Ported from legacy assistant.jsx. Both build CTAs route to
   the /contact wizard (the standalone /journey was removed).
   ============================================================ */
...
interface Mem { goal: string | null; haveSite: string | null; business: string | null; }
interface Rec { website: string | null; automation: string[]; support: string[]; }

function recommend(mem: Mem): Rec {
  const r: Rec = { website: null, automation: [], support: [] };
  const g = mem.goal;
...
function ARecItem({ id }: { id: string }) {
...
function OptBtn({ icon, children, onClick }: { icon?: string; children: ReactNode; onClick: () => void }) {
  return (
...
export function AIAssistant() {
  ...
  const [stage, setStage] = useState<'goal' | 'havesite' | 'business' | 'rec'>('goal');
  const [mem, setMem] = useState<Mem>({ goal: null, haveSite: null, business: null });

  const reset = () => { setStage('goal'); setMem({ goal: null, haveSite: null, business: null }); };
  const pickGoal = (id: string) => { setMem((m) => ({ ...m, goal: id })); setStage((id === 'website' || id === 'unsure') ? 'havesite' : 'business'); };
  const pickHave = (id: string) => { setMem((m) => ({ ...m, haveSite: id })); setStage('business'); };
  const pickBiz = (id: string) => { setMem((m) => ({ ...m, business: id })); setStage('rec'); };

  const goBuild = () => { setOpen(false); router.push('/contact'); };

  const rec = stage === 'rec' ? recommend(mem) : null;
  const recIds = rec ? [rec.website, ...rec.automation, ...rec.support].filter(Boolean) as string[] : [];
          ...
          <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12, maxHeight: 'min(64vh, 500px)', overflowY: 'auto' }}>
            {stage === 'goal' && (
              <>
                ...
                {A_GOALS.map((g) => <OptBtn key={g.id} icon={g.icon} onClick={() => pickGoal(g.id)}>{tr(g.label as Bilingual)}</OptBtn>)}
              </>
            )}
            ...
            {stage === 'havesite' && (
              <>
                ...
                {A_HAVESITE.map((o) => <OptBtn key={o.id} onClick={() => pickHave(o.id)}>{tr(o.label as Bilingual)}</OptBtn>)}
            ...
            {stage === 'business' && (
              <>
                ...
                {A_BUSINESS.map((o) => <OptBtn key={o.id} onClick={() => pickBiz(o.id)}>{tr(o.label as Bilingual)}</OptBtn>)}
            ...
            {stage === 'rec' && rec && (
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 2 }}>
                  <button onClick={goBuild} data-action="assistant-journey" className="btn btn-primary btn-sm full">{t('a_viewjourney')}</button>
                  <button onClick={goBuild} data-action="assistant-proposal" className="btn btn-secondary btn-sm full">{t('assistant_proposal')}</button>
